...
const COMPANIES: CompanyCriteria[] = [
  {
    name: 'Google',
    ...
    <div className="flex-1 bg-slate-950 p-6 md:p-12 relative overflow-hidden animate-slide-up">
      ...
      <div className="mx-auto max-w-4xl space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-400 mt-1">
              Analyze your readiness percentage and check matching expectations for major tech hiring partners.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {COMPANIES.map((company) => {
            ...
            company.skills.forEach((skill) => {
              ...
              if (skillKey === 'oop') {
                const hasOOP = userSkillsLower.includes('oop') ||
            ...
            const totalCriteria = company.skills.length + (company.requiresProjects ? 1 : 0);
              ...
              <Card key={company.name} className="glass-panel p-6 flex flex-col justify-between h-[360px] transition-all duration-300">
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-start gap-4">
                    <div>
                      ...
                      <p className="text-[11px] text-slate-500 line-clamp-2 mt-1">{company.desc}</p>
                    </div>
                    <div className="flex flex-col items-end">
                      <span className="text-xl font-black text-white">{readinessPercent}%</span>
                      <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider">Matched</span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3 pt-2">
                    ...
                    {matchedCount > 0 && (
                      <div className="space-y-1">
                        <span className="text-[9px] font-bold text-emerald-400 uppercase tracking-widest block">Matched Requirements</span>
                        <div className="flex flex-wrap gap-1">
                          {matchedSkills.map((s) => (
                            <span key={s} className="text-[9px] font-semibold px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-400 border border-emerald-500/10">
                              {s}
                            </span>
                          ))}
                        </div>
                    ...
                    {missingSkills.length > 0 ? (
                      <div className="space-y-1">
                        <span className="text-[9px] font-bold text-rose-400 uppercase tracking-widest block">Missing Requirements</span>
                        <div className="flex flex-wrap gap-1">
                          {missingSkills.map((s) => (
                            <span key={s} className="text-[9px] font-semibold px-2 py-0.5 rounded bg-rose-500/10 text-rose-300 border border-rose-500/10">
                              {s}
                            </span>
                          ))}
                        </div>
                      ...
                      <div className="flex items-center gap-1 text-[10px] text-emerald-400 font-bold">
                        <ShieldCheck className="h-4.5 w-4.5" /> Full alignment met! Ready to apply.
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-border/5 flex items-center justify-between text-[10px] text-slate-500 font-medium mt-2">
                  ...
                  {readinessPercent < 100 ? (
                    <Link href="/dashboard/skill-gap" className="text-indigo-400 font-bold hover:text-indigo-300 flex items-center gap-0.5 hover:underline">
                      Close skill gaps
                      <ArrowRight className="h-3 w-3" />
                    </Link>
                  ) : (
                    <span className="text-emerald-400 font-bold">Recommended for scouting</span>
                  )}
                </div>
              </Card>
            );
        ...
        <Card className="glass-panel p-6 flex items-start gap-4">
          ...
          <div className="space-y-1">
            <h4 className="text-sm font-bold text-white">Scouting & Interview Invitations</h4>
            <p className="text-xs text-slate-400 leading-relaxed">
              When your readiness index for a hiring partner hits 80% or higher, your profile is marked as a candidate match. Keep updating your portfolio skills and project completions to boost visibility for top corporate scouts.
            </p>
          </div>
        </Card>
      </div>
