...
export default async function RoadmapPage() {
  ...
  const allSkillsCompleted = roleSkills.every((s) =>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-400 mt-1">
              Personalized pathway mapping optimized for selected target role
              benchmarks.
            </p>
          </div>
        ...
        {!targetRole ? (
          <Card className="glass-panel p-12 text-center space-y-4">
            ...
            <p className="text-slate-400 max-w-md mx-auto text-sm">
              Please configure a Target Job Role in your profile to generate a
              structured timeline roadmap.
            </p>
                ...
                <div className="flex flex-col md:flex-row justify-between md:items-center gap-2">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">
                      Completed {completedStepsCount} of {totalStepsCount}{" "}
            ...
            <Card className="glass-panel p-8">
              <div className="relative border-l-2 border-indigo-500/20 ml-6 md:ml-8 space-y-12 pb-4">
                {roadmapSteps.map((step, index) => {
                  ...
                  const isPreviousStepsCompleted = roadmapSteps
                    .slice(0, index)
                    .every((s) => s.completed);
                  ...
                  return (
                    <div key={index} className="relative pl-10 md:pl-12">
                      ...
                      <div
                        className={`absolute -left-[17px] top-1 flex h-8 w-8 items-center justify-center rounded-full border-2 transition-all duration-300 relative z-10 ${
                          isCompleted
                        ...
                        {isActive && (
                          <div className="mt-2 p-3 rounded-lg bg-slate-950/60 border border-border/5 text-[11px] max-w-md">
                            ...
                            <span className="text-slate-300 font-medium">
                              {step.type === "skill" &&
                                `Add "${step.title}" skill under your Profile settings.`}
                              {step.type === "projects" &&
                                "Create and register at least one technical project on your profile."}
                              {step.type === "interview" &&
                                "Complete mock interviews and prepare data sheets."}
                            </span>
                          </div>
                        )}
                      </div>
            ...
            <Card className="glass-panel p-6 flex items-start gap-4">
              ...
              <div className="space-y-1">
                <h4 className="text-sm font-bold text-white">
                  Milestone Tracking Note
                </h4>
                <p className="text-xs text-slate-400 leading-relaxed">
                  Your learning roadmap automatically synchronizes as you update
                  your career profile. Once a target skill is added, the roadmap
                  transitions that milestone node to completed, boosting your
