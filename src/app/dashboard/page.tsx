...
const COMPANY_REQUIREMENTS = [
  { name: 'Google', skills: ['Data Structures', 'Algorithms', 'System Design', 'Projects'] },
  { name: 'Amazon', skills: ['Data Structures', 'Algorithms', 'DBMS', 'OOP', 'Projects'] },
  { name: 'Microsoft', skills: ['TypeScript', 'SQL', 'Git', 'Algorithms', 'Projects'] },
...
export default async function DashboardPage() {
  ...
  const strongMatchesCount = COMPANY_REQUIREMENTS.map((company) => {
    const matchedCount = company.skills.filter((skill) => {
      ...
      if (sKey === 'oop') {
        return userSkillsLower.includes('oop') ||
    ...
    const score = Math.round(30 + (matchedCount / company.skills.length) * 70);
    return score >= 75;
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">
            Overview statistics of your academic profiles and corporate recruitment preparation.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        ...
        <Card className="glass-panel p-5 flex items-center gap-4 transition-all duration-300">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Readiness Score</p>
          </div>
        </Card>
        ...
        <Card className="glass-panel p-5 flex items-center gap-4 transition-all duration-300">
          ...
          <div>
            <p className="text-2xl font-black text-white">
              {targetRole ? `${acquiredSkills.length} / ${requiredSkills.length}` : `${profile.skills?.length || 0}`}
            </p>
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Skills Acquired</p>
          </div>
        </Card>
        ...
        <Card className="glass-panel p-5 flex items-center gap-4 transition-all duration-300">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Technical Projects</p>
          </div>
        </Card>
        ...
        <Card className="glass-panel p-5 flex items-center gap-4 transition-all duration-300">
          ...
          <div>
            <p className="text-2xl font-black text-white">{strongMatchesCount} / 4</p>
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Company Alignments</p>
          </div>
        </Card>
          ...
          <Card className="glass-panel border border-white/5 bg-slate-900/20 shadow-xl p-6 space-y-4">
            <div>
              <h3 className="text-xs font-bold text-slate-300 uppercase tracking-widest mb-2 flex items-center gap-2">
                <Layers className="h-4 w-4 text-indigo-400" /> Professional Skills
              </h3>
              {profile.skills && profile.skills.length > 0 ? (
                <div className="flex flex-wrap gap-1.5">
                  {profile.skills.map((skill: string) => (
                    <span key={skill} className="text-[10px] px-2 py-0.5 rounded-md bg-indigo-500/10 text-indigo-300 border border-indigo-500/10 font-semibold">
                      {skill}
                    </span>
                  ))}
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card className="glass-panel border border-white/5 bg-slate-900/20 shadow-xl p-6 h-64 flex flex-col justify-between">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-1">Preparation benchmark points inventory.</p>
                </div>
                <span className="text-[10px] font-bold px-2 py-0.5 bg-indigo-500/10 text-indigo-400 rounded-full border border-indigo-500/20">LIVE</span>
              </div>
              ...
              <div className="flex items-center justify-center my-2">
                <div className="relative flex items-center justify-center">
                  ...
                  <div className="flex flex-col items-center justify-center bg-slate-950 h-24 w-24 rounded-full border-4 border-indigo-600/80 shadow-2xl relative z-10">
                    ...
                    <span className="text-[9px] text-slate-500 font-bold uppercase tracking-widest">Ready</span>
                  </div>
                </div>
              </div>
              ...
              <div className="text-xs font-semibold text-indigo-400 hover:text-indigo-300 flex items-center gap-1 cursor-pointer transition-colors justify-end">
                <Link href="/dashboard/readiness" className="flex items-center gap-1">
            ...
            <Card className="glass-panel border border-white/5 bg-slate-900/20 shadow-xl p-6 h-64 flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">Alignment metrics with tech hiring companies.</p>
              </div>
              ...
              <div className="space-y-3 overflow-y-auto pr-1">
                {COMPANY_REQUIREMENTS.map((company) => {
                  ...
                  const percent = Math.round(30 + (matchedCount / company.skills.length) * 70);
                  ...
                  return (
                    <div key={company.name} className="space-y-1">
                      ...
                      <div className="w-full bg-slate-800 rounded-full h-1 overflow-hidden">
                        <div className="bg-gradient-to-r from-violet-600 to-indigo-600 h-full rounded-full" style={{ width: `${percent}%` }} />
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="text-xs font-semibold text-indigo-400 hover:text-indigo-300 flex items-center gap-1 cursor-pointer transition-colors justify-end">
                <Link href="/dashboard/company-readiness" className="flex items-center gap-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card className="glass-panel border border-white/5 bg-slate-900/20 shadow-xl p-6 h-64 flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">
                  {targetRole ? `Key missing skills for ${targetRole}:` : 'Select target role to analyze skill gap.'}
                </p>
              </div>
              ...
              <div className="flex-1 my-3 overflow-y-auto pr-1">
                {targetRole ? (
                  missingSkills.length > 0 ? (
                    <div className="flex flex-wrap gap-1.5">
                      {missingSkills.map((s) => (
                        <span key={s} className="text-[10px] font-semibold px-2.5 py-1 rounded-md bg-rose-500/10 text-rose-300 border border-rose-500/10">
                          {s}
                        </span>
                      ))}
                    </div>
                  ...
                  <p className="text-xs text-slate-500 italic">No target role selected.</p>
                )}
              </div>
            ...
            <Card className="glass-panel border border-white/5 bg-slate-900/20 shadow-xl p-6 h-64 flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">Structured career progress tracking.</p>
              </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-center text-xs font-bold text-slate-300">
                      ...
                      <span className="text-indigo-400">
                        {acquiredSkills.length} / {requiredSkills.length + 2} Steps
                      </span>
                    </div>
                    <div className="w-full bg-slate-800 rounded-full h-2.5 overflow-hidden">
                      <div
                        className="bg-gradient-to-r from-violet-600 via-indigo-600 to-emerald-500 h-full rounded-full"
                        style={{ width: `${Math.round((acquiredSkills.length / (requiredSkills.length + 2)) * 100)}%` }}
                      />
                    </div>
                  </div>
                ) : (
                  <p className="text-xs text-slate-500 italic">Select a target role to unlock dynamic timelines.</p>
                )}
              </div>
