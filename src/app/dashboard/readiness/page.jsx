...
export default async function ReadinessScorePage() {
  ...
  // Scoring Component Calculations (Max 100 points) - Phase 13 Formula
  ...
  if (skillsPoints >= 20)
    strengths.push(
      "Strong alignment with core technologies for the target role",
    );
  if (projectsPoints >= 20)
    strengths.push("Completed multiple hands-on development projects");
  if (dsaProgressVal >= 60)
    strengths.push(
      `Excellent DSA preparation status (${dsaProgressVal}% completed)`,
    );
  if (certsPoints >= 10)
    strengths.push("Holds multiple industry-certified credentials");
  if (profile.cgpa >= 8.0)
    strengths.push(
      `Maintained a high academic scoring background (${profile.cgpa.toFixed(2)} CGPA)`,
    );
  if (strengths.length === 0)
    strengths.push(
      "Account registered successfully and placement tracking activated",
    );
  ...
  if (missingSkills.length > 0) {
    suggestions.push(
      `Acquire core target skills: ${missingSkills.slice(0, 3).join(", ")}`,
    );
  }
  if (projectsCount < 1) {
    suggestions.push(
      "Build one full stack project to highlight development competencies",
    );
  } else if (projectsCount < 2) {
    suggestions.push(
      "Build another advanced repository to expand technical breadth",
    );
  }
  if (dsaProgressVal < 50) {
    suggestions.push(
      "Improve DSA progress (Aim to complete foundational data structures & algorithms)",
    );
  }
  if (certsCount < 1) {
    suggestions.push(
      "Earn an industry-standard certification to validate your skill set",
    );
  }
  if (resumeScoreVal < 80) {
    suggestions.push(
      "Upload/Refine your resume in Resume Analyzer to optimize layout and ATS compatibility",
    );
  }
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-400 mt-1">
              Your preparation score evaluated against technical placement
              requirements.
            </p>
          </div>
              ...
              <div className="space-y-4 mt-4">
                ...
                <div className="flex justify-between items-center text-xs p-3 rounded bg-slate-950/60 border border-border/5">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-[10px] text-slate-500">
                      Acquire core skills for selected target role (Max 30 pts)
                    </p>
                  </div>
                ...
                <div className="flex justify-between items-center text-xs p-3 rounded bg-slate-950/60 border border-border/5">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-[10px] text-slate-500">
                      12.5 pts per project added to profile (Max 25 pts)
                    </p>
                  </div>
                  <span className="font-bold text-indigo-400">
                    {projectsPoints} / 25 pts
                  </span>
                </div>
                ...
                <div className="flex justify-between items-center text-xs p-3 rounded bg-slate-950/60 border border-border/5">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-[10px] text-slate-500">
                      Proportional to DSA curriculum percentage (Max 20 pts)
                    </p>
                  </div>
                  <span className="font-bold text-indigo-400">
                    {dsaPoints} / 20 pts
                ...
                <div className="flex justify-between items-center text-xs p-3 rounded bg-slate-950/60 border border-border/5">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-[10px] text-slate-500">
                      5 pts per credential listing (Max 10 pts)
                    </p>
                  </div>
                  <span className="font-bold text-indigo-400">
                    {certsPoints} / 10 pts
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  missingSkills.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {missingSkills.map((skill) => (
                        <span
                          key={skill}
                          className="inline-flex items-center gap-1.5 text-xs px-3 py-1 rounded bg-rose-500/10 text-rose-300 border border-rose-500/10 font-medium"
                        >
          ...
          <Card className="glass-panel p-6 flex flex-col justify-between">
            <div>
              ...
              <p className="text-xs text-slate-400 mb-4">
                Recommended tasks to directly increase your placement readiness
                score:
              </p>
              ...
              <ul className="text-xs text-slate-300 space-y-2">
                {suggestions.map((s, idx) => (
                  <li
                    key={idx}
                    className="flex items-start gap-2 p-2.5 rounded bg-slate-950/60 border border-border/5"
                  >
        ...
        <Card className="glass-panel p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {strengths.map((s, idx) => (
              <div
                key={idx}
                className="flex items-center gap-2.5 p-3 rounded-lg bg-indigo-500/5 border border-indigo-500/10 text-xs text-slate-300"
              >
